...
def materials_results_html(entity_query, raw_text):
    ...
    if not results:
        ...
        df = pd.DataFrame(
            {"vítima": materials, "count": counts, "dois": dois}
        )
...
def materials_table_html(df, limit):
    ...
    """
    header_material = html.Th("Entidade")
    header_counts = html.Th("Fração de Publicações")
    header_clickable = html.Th(
        f"Links ({MAX_N_DOIS_FOR_VIEWING} clicáveis)"
    )
    header_downloadable = html.Th("Download")
    ...
    for i in range(n_results):
        material_cell = html.Td(df["vítima"].iloc[i])
...
def make_download_link_from_all_dois_html(df, link_text=None):
    ...
    Args:
        df (pd.DataFrame): The dataframe containing columns "vítima", "count",
            and "dois".
    ...
    link = html.A(
        link_text,
        id="download-link",
        # REQ: O arquivo deve conter a data do download para diferenciação
        download="zumbi_data.csv",
        href=csv_string,
        target="_blank",
    )
    return link
